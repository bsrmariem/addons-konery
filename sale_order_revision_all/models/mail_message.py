...
class MailMessage(models.Model):
    ...
    revision_subject_display = fields.Html(string="Subject", compute="_revision_compute_subject_display")
    revision_uid = fields.Many2one(string="Edited by", comodel_name="res.users")
    revision_attachment_count = fields.Integer(
        string="Attachments count", compute="_revision_compute_attachment_count")
    revision_author_display = fields.Char(string="Author", compute="_revision_compute_author_display")
    # Fields to avoid access check issues
    revision_author_allowed_id = fields.Many2one(
        string="Author",
    ...
    def _revision_compute_author_allowed(self):
        ...
        for rec in self:
            ...
            if author_id in author_allowed_ids:
                rec.revision_author_allowed_id = author_id
            else:
                rec.revision_author_allowed_id = False

    # -- Search allowed authors
    @api.model
    def _revision_search_author_allowed(self, operator, value):
        return [("author_id", operator, value)]

    # -- Get Author for tree view
    @api.depends("revision_author_allowed_id")
    def _revision_compute_author_display(self):
        for rec in self:
            rec.revision_author_display = (
                rec.revision_author_allowed_id.name
                if rec.revision_author_allowed_id
                else (rec.email_from or "").replace(">", "").replace("<", "")
            )

    @api.depends("attachment_ids")
    def _revision_compute_attachment_count(self):
        for rec in self:
            rec.revision_attachment_count = len(rec.attachment_ids)
